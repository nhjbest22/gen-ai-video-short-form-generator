...
export class VideoUploadStateMachine extends Construct {
  ...
  constructor(
    scope: Construct,
    id: string,
    props: VideoUploadStateMachineProps
  ) {
    super(scope, id);
    ...
    const extractTimeframe = new ExtractTimeframe(
      this,
      "ExtractTimeframeFunc",
      {
        bucket: props.bucket,
        highlightTable: props.highlightTable,
      }
    ...
    const prepareParameters = new sfn.Pass(this, "PrepareParameters", {
      parameters: {
        "uuid.$":
          "States.Format('{}', States.ArrayGetItem(States.StringSplit($.detail.object.key, '/'), 1))",
        "TranscriptionJobName.$":
          "States.Format('{}_stepFunction', States.ArrayGetItem(States.StringSplit($.detail.object.key, '/'), 1))",
        "raw_file_uri.$":
          "States.Format('s3://{}/{}', $.detail.bucket.name, $.detail.object.key)",
        "bucket_name.$": "$.detail.bucket.name",
        "OutputKey.$":
          "States.Format('videos/{}/Transcript.json', States.ArrayGetItem(States.StringSplit($.detail.object.key, '/'), 1))",
      },
    });
    ...
    const checkTranscriptionJobStatus = new sfn.Choice(
      this,
    ...
    const extractTimeframeTask = new tasks.LambdaInvoke(
      this,
      "ExtractTimeframe",
      {
    ...
    const checkExtractionJobStatus = new sfn.Choice(
      this,
      "CheckExtractionJobStatus"
    );

    const mediaConvertExtractJob = new tasks.MediaConvertCreateJob(
      this,
    ...
    const mediaConvertExtractJobParam = new sfn.Pass(
      this,
      "MediaConvertExtractJobParam",
      {
        parameters: {
          "FHD_Jobname.$":
            "States.Format('{}_FHD_{}', $.timeframe_extracted.uuid, $.timeframe_extracted.index)",
          "FHD_SourceKey.$":
            "States.Format('{}.mp4', $.timeframe_extracted.output_destination)",
          "FHD_OutputKey.$":
            "States.Format('videos/{}/ShortsTranscript/{}-TranscriptShorts.json', $.timeframe_extracted.uuid, $.timeframe_extracted.index)",
        },
        resultPath: "$.FHD_Job",
      }
    );

    const startHighlightTranscriptionJob = new tasks.CallAwsService(
      this,
      "StartHighlightTranscriptionJob",
      {
        service: "transcribe",
        action: "startTranscriptionJob",
        iamAction: "transcribe:StartTranscriptionJob",
    ...
    const waitForHighlightTranscriptionJob = new sfn.Wait(
      this,
      "WaitForHighlightTranscriptionJob",
      {
        time: sfn.WaitTime.duration(Duration.seconds(5)),
      }
    );

    const getHighlightTranscriptionJobStatus = new tasks.CallAwsService(
      this,
      "GetHighlightTranscriptionJobStatus",
      {
        service: "transcribe",
        action: "getTranscriptionJob",
        iamAction: "transcribe:GetTranscriptionJob",
    ...
    const checkHighlightTranscriptionJobStatus = new sfn.Choice(
      this,
      "CheckHighlightTranscriptionJobStatus"
    );
    ...
    const definitionBody = prepareParameters
      ...
      .next(processTopicsMap.itemProcessor(processTopicTask))
      .next(updateDDB(2))
      .next(updateEvent(2))
      .next(
        highlightExtractMap.itemProcessor(
          extractTimeframeTask.next(
            checkExtractionJobStatus
              .when(
                sfn.Condition.numberEquals(
                  "$.timeframe_extracted.statusCode",
                  200
                ),
                mediaConvertExtractJob
                  ...
                  .next(
                    checkHighlightTranscriptionJobStatus
                      .when(
                        sfn.Condition.stringEquals(
                          "$.highlightJobStatus.TranscriptionJob.TranscriptionJobStatus",
                          "COMPLETED"
                        ),
